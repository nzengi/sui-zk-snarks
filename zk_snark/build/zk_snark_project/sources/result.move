module zk_snark::result {
    ...
    fun test_verify_with_result() {
        ...
        test_scenario::next_tx(scenario, admin);
        {
            ...
            let alpha = vector[1u8, 2u8, 3u8];
            let beta = vector[4u8, 5u8, 6u8];
            let gamma = vector[7u8, 8u8, 9u8];
            let delta = vector[10u8, 11u8, 12u8];
            let ic = vector[vector[13u8, 14u8, 15u8]];
            
            admin::update_verification_key(&admin_cap, &mut vk, alpha, beta, gamma, delta, ic);
